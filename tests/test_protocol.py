def test_construct_state_change(protocol):
    assert protocol.construct_state_change(1) == bytearray([0xBB, 0x0F, 0x69, 0x52, 0x00, 0x41, 0x06, 0x01])
    assert protocol.construct_state_change(0) == bytearray([0xBB, 0x0F, 0x69, 0x52, 0x00, 0x41, 0x06, 0x00])

def test_construct_levels_change(protocol):
    assert ( protocol.construct_levels_change(0,0,0,0,0,0,0,0) == 
    ...
    assert ( protocol.construct_levels_change(0x111,0x222,0x333,0x444,0x555,0x666,0x777,0xFFF) == 
        bytearray([0xBB, 0x0F, 0x69, 0x52, 0x00, 0x41, 0x11, 0xFF,
...
def test_construct_effect_start(protocol):
    assert ( protocol.construct_effect_start(0) == 
        bytearray([0xBB, 0x0F, 0x69, 0x52, 0xC0, 0x01, 0x01, 0x00]))
    assert ( protocol.construct_effect_start(5) == 
